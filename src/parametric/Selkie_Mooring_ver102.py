...
from utilities import mkdir, angleGenerator
...
def system(mooring):
    ...
    ms.lineTypes[typeName] = getLinePropsFunSelkie(rank, typeName, 
        mooring['moorConfig']['moorType']['grade'], 
        mooring['moorConfig']['moorType']['stud'], 
    ...
    return ms, anc_radius, line_length, typeName


def period_test(Tn_limit, M, ma_inf, k):
    Tn = np.diag(2 * np.pi * np.sqrt((M + ma_inf)/k))
...
def Moor_CreateV2(mooring, num_lines, rank, out_direc = None):
    ...
    if mooring['moorConfig']['moorType']['Type'] == "Cat":
        ms.lineTypes[typeName] = getLinePropsFunSelkie(rank, typeName, 
        source = mooring['moorConfig']['moorType']['source'], 
        ...
        #depth_fair_seabed*math.sqrt(2*(MBL/(mass_wet*g*depth_fair_seabed))-1)
        #line_length= depth_fair_seabed*math.sqrt(np.abs(2*(MBL/(mass_wet*g*depth_fair_seabed))-1))
        
        #alpha = (mooring['moorConfig']['PreTen_Ratio']*MBL)/(mass_wet*g)
        anc_radius= mooring['moorConfig']['moorType']['anchor_radius']
        #line_length-depth_fair_seabed*math.sqrt(1+(2*alpha/depth_fair_seabed))+alpha*np.arccosh(1+depth_fair_seabed/alpha)
        
        #check limit for radius
        

            
# =============================================================================
#             if anc_radius > rmax or anc_radius < rmin:
#                 raise ('The values of line length and anchor radius ')
# =============================================================================
    elif mooring['moorConfig']['moorType']['Type'] == "Tau":
        ms.lineTypes[typeName] = getLinePropsFunSelkie(rank, typeName, 
        source = mooring['moorConfig']['moorType']['source'],
        ...
        #line_length_unstr = (EA*line_length_str - pretension*line_length_str)/EA
        #line_length = line_length_unstr
        anc_radius= mooring['moorConfig']['moorType']['anchor_radius']#mooring['moorConfig']['fair_radius'] + math.sqrt(line_length_str**2-depth_fair_seabed**2)
    else: 
        print('Error, either input moor_type as "Cat" or "Tau"')
...
# =============================================================================
# 
#     # Plot and save mooring system figure in unloaded condition 
#     plot_on = 0
#     if plot_on == 1:
#         fig, ax = ms.plot() 
#         plt.savefig('Test1_NoLoad.png', dpi = 300)
#         ms.plot(colortension=False, cbar_tension=False)
#         plt.savefig('Test1_NoLoad_T.png', dpi = 300)
# 
# 
# =============================================================================
                                        # plot the system in original configuration
    #
...
# =============================================================================
#     if plot_on == 1:
#         fig, ax = ms.plot(color='blue')    # plot the system in displaced configuration (on the same plot, in red)
#         plt.savefig('Test1_MeanLoad.png', dpi = 300)
#         ms.plot(colortension=True, cbar_tension=True)
#         plt.savefig('Test1_MeanLoad_T.png', dpi = 300)
#         
# =============================================================================
